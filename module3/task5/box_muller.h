...
namespace point {
    ...
    double get_weight(const pdd &a, const pdd &b) {
        return std::sqrt(std::pow((a.first - b.first), 2) + std::pow((a.second - b.second), 2));
    }

    double get_weight(const std::vector<pdd> &points, int from, int to) {
        return ((from != to) ? get_weight(points[from], points[to]) : 0);
    }
}
